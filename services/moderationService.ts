...
import { analyzeContentWithGemini } from './geminiService';
...
export const scanText = async (text: string): Promise<ModerationResult | null> => {
    if (!text.trim()) return null;

    try {
        const resultJson = await analyzeContentWithGemini(TEXT_MODERATION_PROMPT, { text });
...
export const scanImage = async (imageBlob: Blob): Promise<ModerationResult | null> => {
    try {
