...
const CreatorProfileModal: React.FC<CreatorProfileModalProps> = ({ creator, currentUser, onClose, onFollow }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4">
            <div className="bg-[--bg-primary] rounded-lg shadow-xl w-full max-w-sm relative border border-[--border-color]">
                ...
                <div className="p-8 flex flex-col items-center text-center">
                    ...
                    {creator.id !== currentUser.id && (
                        <button
                            ...
                            className={`w-full mt-6 px-4 py-2 rounded-full text-sm font-semibold transition-colors ${isFollowingCreator ? 'bg-[--accent-secondary] text-white' : 'bg-[--bg-tertiary] text-[--text-primary] hover:bg-[--bg-hover]'}`}
                        >
