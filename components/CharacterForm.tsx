        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1">
                ...
                <div className="mt-2 flex flex-col items-center gap-4">
                    <img src={previewUrl || defaultAvatar} alt="Avatar Preview" className="w-48 h-48 rounded-lg object-cover bg-gray-700"/>
                    <div className="flex items-center gap-2">
                        <label htmlFor="avatar-upload" className="cursor-pointer bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-md inline-flex items-center gap-2 text-sm">
                            <UploadIcon className="w-4 h-4" />
                            <span>Upload</span>
                        </label>
                        <input id="avatar-upload" type="file" className="hidden" accept="image/*" onChange={handleFileChange} />
                        <button type="button" onClick={handleRemoveAvatar} className="bg-red-800 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md inline-flex items-center gap-2 text-sm">
                ...
                <div>
                 ...
                 <div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-gray-300">
                        <label className="flex items-center"><input type="radio" name="gender-radio" value="male" checked={character.gender === 'male'} onChange={handleGenderChange} className="form-radio text-pink-500 bg-gray-700" /> <span className="ml-2">Male</span></label>
                        <label className="flex items-center"><input type="radio" name="gender-radio" value="female" checked={character.gender === 'female'} onChange={handleGenderChange} className="form-radio text-pink-500 bg-gray-700"/> <span className="ml-2">Female</span></label>
                        <label className="flex items-center"><input type="radio" name="gender-radio" value="non-binary" checked={character.gender === 'non-binary'} onChange={handleGenderChange} className="form-radio text-pink-500 bg-gray-700"/> <span className="ml-2">Non-binary</span></label>
                        <label className="flex items-center"><input type="radio" name="gender-radio" value="unspecified" checked={character.gender === 'unspecified'} onChange={handleGenderChange} className="form-radio text-pink-500 bg-gray-700"/> <span className="ml-2">Unspecified</span></label>
                    </div>
                </div>
                <div>
        ...
        <div className="flex items-center justify-start pt-6">
          <label htmlFor="isPublic" className="flex items-center cursor-pointer">
            <div className="relative">
              ...
              <div className={`block w-14 h-8 rounded-full ${character.isPublic ? 'bg-green-500' : 'bg-gray-600'} ${auth?.currentUser?.isSilenced ? 'opacity-50 cursor-not-allowed' : ''}`}></div>
            ...
            <div className="ml-3 text-gray-300 font-medium">
              ...
              <p className="text-xs text-gray-500">{character.isPublic ? 'Visible to everyone on the home page.' : 'Only visible to you on your profile.'}</p>
