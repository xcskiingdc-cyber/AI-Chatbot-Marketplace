...
const ChatSettingsModal: React.FC<ChatSettingsModalProps> = ({ isOpen, onClose, settings, onSave, onResetChat, userType }) => {
  ...
  const formFieldClasses = "w-full p-2 bg-[--bg-secondary] border border-[--border-color] rounded-md focus:outline-none focus:ring-2 focus:ring-[--accent-primary] disabled:opacity-50 disabled:cursor-not-allowed";
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4">
        <div className="bg-[--bg-primary] rounded-lg shadow-xl w-full max-w-md relative border border-[--border-color]">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="relative">
                <input type="checkbox" id="isStreaming" name="isStreaming" 
                       ...
                       className="sr-only" />
                <div className={`block w-14 h-8 rounded-full ${currentSettings.isStreaming ? 'bg-[--accent-primary]' : 'bg-[--bg-tertiary]'}`}></div>
          ...
          <div className="p-4 bg-[--bg-secondary]/50 border-t border-[--border-color] flex justify-end gap-4">
            <button type="button" onClick={onClose} className="px-6 py-2 bg-[--bg-tertiary] hover:bg-[--bg-hover] rounded-md transition-colors">Cancel</button>
