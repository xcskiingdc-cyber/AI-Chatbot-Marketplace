...
const ChatSettingsModal: React.FC<ChatSettingsModalProps> = ({ isOpen, onClose, settings, onSave, onResetChat, userType }) => {
  ...
  const formFieldClasses = "w-full p-2 bg-gray-800 border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-pink-500 disabled:opacity-50 disabled:cursor-not-allowed";
        ...
        <div className="bg-gray-900 rounded-lg shadow-xl w-full max-w-md relative border border-gray-700">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-between">
              <label htmlFor="isStreaming" className="font-medium text-gray-300">
                Streaming Replies
                <p className="text-xs text-gray-500">Receive replies word-by-word.</p>
              </label>
              <div className="relative">
                <input type="checkbox" id="isStreaming" name="isStreaming" 
                       ...
                       className="sr-only" />
                <div className={`block w-14 h-8 rounded-full ${currentSettings.isStreaming ? 'bg-pink-600' : 'bg-gray-600'}`}></div>
            ...
            <div>
              ...
              <select 
               ...
               {!canUsePro && <p className="text-xs text-gray-500 mt-1">Upgrade to a Subscription or Ad-supported plan to use Pro models.</p>}
            </div>
          ...
          <div className="p-4 bg-gray-950/50 border-t border-gray-700 flex justify-end gap-4">
            <button type="button" onClick={onClose} className="px-6 py-2 bg-gray-600 hover:bg-gray-500 rounded-md transition-colors">Cancel</button>
            <button type="button" onClick={handleSave} className="px-6 py-2 bg-blue-600 hover:bg-blue-500 rounded-md transition-colors">Save Settings</button>
          </div>
        </div>
      </div>
      {isResetModalOpen && (
